...
export class AppVersionService {
  ...
  public checkAppVersion() {
    let thisVersionNumber$ = Observable.fromPromise(
      this._appVersion.getVersionNumber()
    );
    let latestAppRelease$ = this._dataService.getAppVersion();
    let appActivated$ = this._platform.resume.merge(
      Observable.fromPromise(this._platform.ready())
    );
    ...
    this._storageService.lastNotifiedAppVersion$
      ...
      .subscribe(
        latestValues => {
          ...
          console.log(
            "AppVersionService: thisAppVersionNumber",
            thisAppVersionNumber
          );
          console.log(
            "AppVersionService: lastNotifiedAppVersionNumber",
            this._lastNotifiedVersionNumber
          );
          console.log(
            "AppVersionService: latestAppReleaseVersionNumber",
            latestAppReleaseVersionNumber
          );
          console.log(
            "AppVersionService: latestAppReleaseHardRelease",
            latestAppReleaseHardRelease
          );

          if (
            latestAppReleaseVersionNumber > thisAppVersionNumber &&
            latestAppReleaseHardRelease
          ) {
