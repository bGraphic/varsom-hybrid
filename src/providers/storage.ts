...
export class StorageService {
  ...
  constructor(
    ...
    platform.ready().then(() => {
      ...
      this._loadValues(storage, this.STORE_KEY, this._storedValues$, this._defaultValues);
    });

  }

  private _saveOnPause(platform: Platform, storage: Storage) {
    Observable.combineLatest(this._storedValues$, this._newValues$, platform.pause)
      .subscribe(latest => {
        const [storedValues, newValues] = latest;
        this._saveValues(storage, this.STORE_KEY, Object.assign({}, storedValues, newValues));
      });
  }

  private _saveContinously(storage: Storage) {
    Observable.combineLatest(this._storedValues$, this._newValues$)
      .subscribe(latest => {
        const [storedValues, newValues] = latest;
        this._saveValues(storage, this.STORE_KEY, Object.assign({}, storedValues, newValues));
      });
