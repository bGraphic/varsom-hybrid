...
export class DataService {
  ...
  constructor(private _db: AngularFireDatabase, private _platform: Platform) {
  ...
  getForecastForRegions(forecastType: string): Observable<any[]> {
    if ("avalanche" === forecastType) {
      return this._getForecasts(
        "/forecast/avalanche/regions/",
        this._avalancheCache$,
        0
      );
    } else {
      console.error(
        "DataService: Regions can only have avalanche forecasts, not",
        forecastType
      );
    }
  }
  ...
  getForecastForCounties(forecastType: string): Observable<any[]> {
    ...
    if ("flood" === forecastType) {
      ...
      console.error(
        "DataService: Counties can only have flood/landslide forecasts, not",
        forecastType
      );
    }
  }

  getForecastForMunicipalities(
    forecastType: string,
    parentId: string
  ): Observable<any[]> {
    ...
    if ("flood" === forecastType) {
      ...
      console.error(
        "DataService: Municipalities can only have flood/landslide forecasts, not",
        forecastType
      );
    }
  }
  ...
  getAppVersion(): Observable<{ versionNumber: string; hard: boolean }> {
    return this._getObject("/api/app_release").map(appRelease => {
      return {
        versionNumber: appRelease.version_number,
        hard: appRelease.hard
      };
    });
  }
  ...
  addPushTokenForArea(pushToken: string, areaId: string) {
    ...
    item.set(moment().format()).catch(error => {
      console.log(
        "DataService: Error saving push token",
        pushToken,
        areaId,
        error
      );
  ...
  removePushTokenForArea(pushToken: string, areaId: string) {
    ...
    console.log(
      "DataService: Removing push token from area",
      pushToken,
      areaId
    );
    let item = this._db.object("/subscriptions/id" + areaId + "/" + pushToken);
    item.remove().catch(error => {
      console.log(
        "DataService: Error saving push token",
        pushToken,
        areaId,
