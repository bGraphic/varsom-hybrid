...
export class FavoriteService {
  ...
  private _favoriteAreasWithStatus$ = new BehaviorSubject<
    { areaId: string; active: boolean }[]
  >([]);
  private _favoriteChanges$ = new Subject<{
    type: string;
    areaIds: string[];
  }>();

  get favoriteAreaIds$(): Observable<string[]> {
    return this._favoriteAreasWithStatus$.map(items =>
      this._transformsItemsToActiveAreaIds(items)
    );
  }
  ...
  isFavoriteArea$(areaId): Observable<boolean> {
    return this.favoriteAreaIds$.map(favorites => {
  ...
  private _watchChangesToFavoriteAreaIds() {
    this._favoriteChanges$.subscribe(change => {
      console.log(
        "FavoriteService: Change in favorites: ",
        change.type,
        JSON.stringify(change.areaIds)
      );

      let areas = this._favoriteAreasWithStatus$.getValue();
      let setToActive = "REMOVE" !== change.type;
      let overrideExistingStatus =
        "ADD" === change.type || "REMOVE" == change.type; // Do not override when change type is PARSE or SAVED

      areas = this._changeStatusOfAreas(
        areas,
        change.areaIds,
        setToActive,
        overrideExistingStatus
      );
  ...
  private _watchInitialStoredFavoriteValuesAndSaveChanges() {
    this._storageService.favorites$.subscribe(favorites => {
      if (!this._pushToken$.getValue()) {
        this._pushToken$.next(favorites.pushToken);
      }

      this._favoriteChanges$.next({
        type: "SAVED",
        areaIds: favorites.areaIds
      });
    });

    this.favoriteAreaIds$.merge(this.pushToken$).subscribe(value => {
      this._storageService.favorites = {
        pushToken: this._pushToken$.getValue(),
        areaIds: this._transformsItemsToActiveAreaIds(
          this._favoriteAreasWithStatus$.getValue()
        )
      };

      this._updateSubscriptions(
        this._favoriteAreasWithStatus$.getValue(),
        this._pushToken$.getValue()
      );
    });
  }

  private _transformsItemsToActiveAreaIds(
    items: { areaId: string; active: boolean }[]
  ) {
    let activeItems = items.filter(item => {
      return item.active;
    });
    let transformedItems = activeItems.map(item => {
      return item.areaId;
    });
  ...
  private _updateSubscriptions(
    areasWithStatus: { areaId: string; active: boolean }[],
    token: string
  ) {
    console.log(
      "FavoriteService: Update subscriptions",
      token,
      JSON.stringify(areasWithStatus)
    );
  ...
  private _changeStatusOfAreas(
    areas: { areaId: string; active: boolean }[],
    areaIds: string[],
    active: boolean,
    overrideExisting = true
  ): { areaId: string; active: boolean }[] {
    console.log(
      "FavoriteService: Change active state of areas to",
      active,
  ...
  private _importParseFavorited() {
    this._storageService.parseImportDone$
      ...
      .concatMap(isImported => {
        ...
        return this.pushToken$.filter(token => {
          return !!token;
        });
      })
      .concatMap(token => {
