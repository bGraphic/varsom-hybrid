...
@Component({
  ...
  template: '<svg #svg class="nve-avalanche-rose" xmlns="http://www.w3.org/2000/svg"></svg>',
})
export class AvalancheRose {
  ...
  drawAvalacheRose() {
    ...
    container.appendChild(this.createCake(radius, cx, cy, this.validExpositions.toString().split('')));

    container.appendChild(this.createLabel(cx, cy - radius, 0.2 * radius, 'N'));
    container.appendChild(this.createLabel(cx + radius, cy, 0.2 * radius, 'Ø'));
    ...
    if (this.exposedHeightFill === 1) {
      container.appendChild(this.createMountain(cx + 1.3 * radius, cy, radius, this.exposedHeightFill));
      container.appendChild(this.createUpArrow(cx + 2.8 * radius, cy + radius - 30, 11.92, 16));
      let label = this.createHeightLabel(cx + 2.8 * radius + 11.92 / 2, cy + radius, radius, this.exposedHeight1 + 'm');
      label.classList.add('centered');
      container.appendChild(label);
    } else if (this.exposedHeightFill === 2) {
      container.appendChild(this.createMountain(cx + 1.3 * radius, cy, radius, this.exposedHeightFill));
      container.appendChild(this.createDownArrow(cx + 2.8 * radius, cy + radius - 16, 11.92, 16));
      let label = this.createHeightLabel(cx + 2.8 * radius + 11.92 / 2, cy + 16, radius, this.exposedHeight1 + 'm');
      label.classList.add('centered');
      container.appendChild(label);
    } else if (this.exposedHeightFill === 3) {
      container.appendChild(this.createMountain(cx + 1.3 * radius, cy - 6, radius, this.exposedHeightFill));
      container.appendChild(this.createUpArrow(cx + 2.7 * radius, cy + radius - 40, 11.92, 16));
      container.appendChild(this.createDownArrow(cx + 2.7 * radius, cy + radius - 16, 11.92, 16));
      container.appendChild(this.createHeightLabel(cx + 2.8 * radius + 16, cy + 10, radius, this.exposedHeight1 + 'm'));
      container.appendChild(this.createHeightLabel(cx + 2.8 * radius + 16, cy + radius - 8, radius, this.exposedHeight2 + 'm'));
    } else if (this.exposedHeightFill === 4) {
      container.appendChild(this.createMountain(cx + 1.3 * radius, cy - 6, radius, this.exposedHeightFill));
      container.appendChild(this.createDownArrow(cx + 2.7 * radius, cy + radius - 48, 11.92, 16));
      container.appendChild(this.createUpArrow(cx + 2.7 * radius, cy + radius - 16, 11.92, 16));
      container.appendChild(this.createHeightLabel(cx + 2.7 * radius, cy + 16, radius, this.exposedHeight2 + '-' + this.exposedHeight1 + 'm'));
    }
  ...
  createMountain(x, y, width, exposedHeightFill) {
    ...
    if (exposedHeightFill == 1) {
      ...
      let bottom = document.createElementNS('http://www.w3.org/2000/svg', 'path');
      ...
      let middle = document.createElementNS('http://www.w3.org/2000/svg', 'path');
